...
bool
response_checker::has_scalar_type(size_t _results_index, size_t _output_index, const UA_DataType* _type) {
...
size_t
response_checker::get_results_size() {
    return response_.resultsSize;
}

size_t
response_checker::get_output_arguments_size(size_t _results_index) {
...
void*
response_checker::get_data(size_t _results_index, size_t _output_index) {
    if (_output_index >= get_output_arguments_size(_results_index)) {
        throw std::invalid_argument("output_index is out of range");
        return NULL;
    }
...
UA_StatusCode
response_checker::get_service_result() {
    return response_.responseHeader.serviceResult;
}