...
method_node_caller::method_node_caller() : is_method_node_called_(false) {
}
...
method_node_caller::call_method_node(UA_Client* _client, UA_NodeId _method_node_id, UA_ClientAsyncCallCallback _callback, void* _userdata) {
    ...
    if(is_method_node_called_) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "Method node already called");
        return status_code;
    }
    status_code = UA_Client_call_async(_client, UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER), _method_node_id, input_arguments_.size(), input_arguments_.data(), _callback, this, NULL);
    is_method_node_called_ = true;
    return status_code;
}