...
int main (int argc, char* argv[]) {
    if(argc != 4) {
      std::cerr << "Usage: " + std::string(argv[0]) + " <robot_count> <absolute_results_directory_path> <result_file_name>\n";
    ...
    result_filename += "-" + std::to_string(robot_count);
    ...
    std::unique_ptr<statistics_writer> sw(statistics_writer::get_instance(robot_count, absolute_results_directory_path, result_filename));
    sw->write_statistics();
    return 0;
}